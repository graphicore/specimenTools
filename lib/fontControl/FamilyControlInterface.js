define([
    ...
    function FamilyControlInterface(container, pubSub) {
    ...
    var _p = FamilyControlInterface.prototype;
    ...
    _p._makeFamilyElement = function(familyIndex, familyName, weightDict) {
        ...
        if(familyHasTwoStyles) {
            ...
            result.italicSwitch = doc.createElement('button');
            result.italicSwitch.textContent = 'italic';
            result.italicSwitch.addEventListener('click', this._switchItalic.bind(this));
            result.element.appendChild(result.italicSwitch);
        }
        ...
        for(i=0,l=weights.length;i<l;i++) {
            ...
            label = weight + ' ' + weight2weightNames[weight];
    ...
    _p._setItalicSwitch = function(italicSwitch, enabled, checked) {
        italicSwitch.disabled = !enabled;
        italicSwitch.classList[checked ? 'add' : 'remove']('checked');
    };
