define([
    ...
    FontsData.getCharSetCoverage = function(testForChars /*string*/
        ...
        var found = 0
          ...
          , laxSkipped = []
          , charCode
            // maybe all inputs should just be sets of charCodes at this point
          , notInTest = []
          , testForSet = new Set()
          ;
        for(i=0;i<l;i++) {
            charCode = testForChars.codePointAt(i);
            testForSet.add(charCode);
        ...
        charset.forEach(function(charCode){
            if(!testForSet.has(charCode))
                notInTest.push(charCode);
        });
               // 0          1      2      3        4         5           6
        return [found/total, found, total, missing, included, laxSkipped, notInTest];
    };
    ...
    _p.pruneCaches = function() {
        var key;
        for(key in this._data.globalCache)
            delete this._data.globalCache[key];
        // this._data.globalCache is not enumerable, so it won't
        // appear in the following loop
        for(key in this._data)
            this._data[key].cache = Object.create(null);
    };

    _p.setOption = function(key, value) {
    ...
    _p.__getCharSetInfo = function (coverage, coveredLanguages, name) {
        ...
        return {
          ...
          , notInCharset: coverage ? coverage[name][6]: []
    ...
    _p._getFamilyName  = function(fontIndex) {
        ...
        if (typeof this._options.overwrites === "object"
                && typeof this._options.overwrites[fontFamily] === "string") {
    ...
    _p.getUseLaxDetection = function() {
        return !!this._options.useLaxDetection;
    };

    _p.getLaxCharList = function() {
        var cacheKey = 'getLaxCharList';
        if(cacheKey in this._data.globalCache)
            return this._data.globalCache[cacheKey];

        var charList = [];
        FontsData.getLaxCharSet(this._options.useLaxDetection)
                    .forEach(function(item) {charList.push(item);});
        charList.sort();
        this._data.globalCache[cacheKey] = charList;
        return charList;
    };
