define([
    ...
    FontsData.getLaxCharSet = function(chars) {
        ...
        if(typeof useLaxDetection === 'string') {
            ...
            for(i=0,l=chars.length;i<l;i++)
                laxCharSet.add(chars.codePointAt(i));
        }
        else
    ...
    FontsData.getGFCoverageInfo = function(gfCharSets, charset, useLaxDetection) {
        var charSets = Object.create(null)
          , k
          ;
        // prepare
        for(k in gfCharSets)
            charSets[k] = gfCharSets[k][0];

        return FontsData.getCoverageInfo(charSets, charset, useLaxDetection);
    };

    FontsData.getLanguageCoverage = function getLanguageCoverage(languageCharSets, font, useLaxDetection) {
    ...
    _p._getLanguageCoverage = function(fontIndex) {
        ...
        return FontsData.getLanguageCoverage(languageCharSets, this._data[fontIndex].font, this._options.useLaxDetection);
    };
