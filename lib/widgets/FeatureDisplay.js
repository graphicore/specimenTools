define([
  ...
  , DiagramRenderer
    ...
    FeatureDisplay.defaultOptions = {
      ...
      , itemContentTextClass: 'feature-display__item__content_text'
      , highlightClasses: 'feature-display__item_highlight'
      , tabularClasses: 'feature-display__item_tabular'
      , itemContentStackedClass: 'feature-display__item__content_stacked'
...
feature items can represent one and more features
...
*/
    ...
    _p._textTabularContent = function(doc, text) {
        var i, l, node
          , parent = doc.createDocumentFragment()
          ;
        for(i=0,l=text.length;i<l;i++) {
    ...
    _p._textTypeFactory = function(contenSetup, contentTemplate, fontIndex) {
        ...
        function setContent(item, i) {
            ...
            var content = contenSetup.behavior === 'tabular'
                ? this._textTabularContent(this._container.ownerDocument, contenSetup.content)
                : this._simpleMarkup(this._container.ownerDocument, contenSetup.content)
                ;
        ...
        if(contenSetup.behavior !== 'show-before') {
        ...
        _mapToClass(element, this._options.itemAppliedClass, function(item, i) {
            /*jshint unused:vars*/
            _setFeaturesToElementStyle(item, contenSetup.features);
        }, this, true);
    ...
    _p._runContentFactory = function(contenSetup, contentTemplate, fontIndex) {
        var factory = this._getContentFactory(contenSetup);
        return factory.call(this, contenSetup, contentTemplate, fontIndex);
    };
    ...
    _p._makeElementFromBluePrint = function(bluePrintNode) {
        ...
        return {
          ...
          , contentTemplates: contentTemplates
                          // <!-- contents -->
          , insertionMarker: getMarkerComments('contents', element, true)
        };
    };
    ...
    _p._buildFeatureItem = function(setup, bluePrintNode, fontIndex) {
        var elementData = this._makeElementFromBluePrint(bluePrintNode)
          ...
          , i,l, contenSetup, contentTemplate, contentItem
        ...
        _mapToClass(element, this._options.itemFriendlyNameClass, function(item, i) {
            /*jshint unused:vars*/
            item.textContent = setup.features.map(function(tag) {
        ...
        for(i=0,l=setup.contents.length;i<l;i++) {
            ...
            if(!contentTemplate)
                continue;
            contentItem = this._runContentFactory(contenSetup
        ...
        if(contents.length) {
            if(elementData.insertionMarker)
                element.insertBefore(contentsFragment, elementData.insertionMarker);
            else
