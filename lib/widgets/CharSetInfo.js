define([
  ...
  , domTool
    ...
    function CharSetInfo(container, pubSub, fontsData, webFontProvider
                                            , widgetsAPI, options) {
    ...
    CharSetInfo.defaultOptions = {
      ...
      , itemTabsTabClass: 'mdl-tabs__tab'
      , itemTabsPanelClass: 'mdl-tabs__panel'
      , fallbackIdHead: 'char-set-info'
    };
    ...
    _p._renderCharSetInfo = function(fontIndex, bluePrintNode, charSetInfo
        ...
        this._activateSleeperClasses(element);

        domTool.mapToClass(element, this._options.itemTabsTabClass
                                                    , function(item, i) {
            /*jshint unused:vars, validthis:true*/
            var id = [this._idHead, charsetIndex, item.getAttribute('data-tab-id')].join('-');
            item.setAttribute('href', '#' + id);
        }, this, false);

        domTool.mapToClass(element, this._options.itemTabsPanelClass
                                                    , function(item, i) {
            /*jshint unused:vars, validthis:true*/
            item.id = [this._idHead, charsetIndex, item.getAttribute('data-tab-id')].join('-');
        }, this, false);
    ...
    _p._activateSleeperClasses = function(element) {
        ...
        while( (item = items.pop()) ) {
            ...
            domTool.applyClasses(item, classes);
        }
    };
    ...
    _p._renderToBlueprintNode = function(fontIndex, insertionMarker
        ...
        for(i=0,l=data.length;i<l;i++) {
            element = this._renderCharSetInfo(fontIndex, bluePrintNode, data[i], isLax, i);
    ...
    _p._onActivateFont = function(fontIndex) {
        ...
        if(this._createdElements !== null) {
            if(this._widgetsAPI)
                this._widgetsAPI.destroy(this._createdElements);
        ...
        if(this._widgetsAPI)
            this._widgetsAPI.init(this._createdElements);
    };
