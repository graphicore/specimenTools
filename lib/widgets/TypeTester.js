define([
  ...
  , OTFeatureInfo
     ...
     *              `fontSizeslider_defaultMin`
     *              `fontSizeslider_defaultMax`
     *              `fontSizeslider_defaultValue`
     *              `fontSizeslider_defaultStep`
     *              `fontSizeslider_defaultUnitType`
     ...
     * @param cssName: e.g. "font-size" or "letter-spacing"
     */
    _p._getSliderOptions = function (cssName) {
        var dataOption
          , sliderDataOptions = ['min', 'max', 'unit', 'step', 'value']
          , sliderName = this._cssName2jsName(cssName)
          , i
          ;
        ...
        for(i=0;i<sliderDataOptions.length;i++) {
            ...
            dataOption = [sliderDataOptions[i]
                    , cssName.substr(0,1).toUpperCase()
                    , cssName.substr(1)
                ].join('');

            // fill options from data values and fallback to defaults
            if(typeof this._container.dataset[dataOption] !== 'undefined')
    ...
    _p._initSlider = function(element) {
        ...
        this._controls[controlsIndex].push(element);
    ...
    _p._initControls = function() {
        ...
        for(i=0;i<sliders.length;i++) {
            ...
            if (!(sliderPropertyName in setup)) {
                this._getSliderOptions(cssName);
                setup[sliderPropertyName] = [
                    [sliders[i]]
                    , ['_initSlider']
                ];
            } else
        ...
        for(i=0;i<labels.length;i++) {
            ...
            if (!(labelPropertyName in setup)) {
                setup[labelPropertyName] = [[labels[i]], ['_initLabel']
    ...
    _p._setCssValue = function(property, value) {
        ...
        for(i=0,l=this._controls[propertyJsName + '_slider'].length;i<l;i++) {
            this._options.setCssValueToInput.call(
                this, this._controls[propertyJsName + '_slider'][i].childNodes[0], value
            );
        }

        // loop over all labels and update their text content
        for(i=0,l=this._controls[propertyJsName + '_label'].length;i<l;i++) {
            this._controls[propertyJsName + '_label'][i].textContent =
                valueUnit.join(' ');
        }
