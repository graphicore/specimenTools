define([
  ...
  , OTFeatureInfo
    ...
    _p._sliderInputHandler = function(evt) {
        this._setCssValue(evt.target.parentElement.dataset.targetProperty, evt.target.value);
        this._applyValues();
    };
    ...
    _p._initSlider = function(element) {
        var input = this._container.ownerDocument.createElement('input')
          , k
          , attributes = {
              min:  ( this._options[ this._cssName2jsName(element.dataset.targetProperty) + '_min'] || this._options['rangeInput_min'] )
            , max:  ( this._options[ this._cssName2jsName(element.dataset.targetProperty) + '_max'] || this._options['rangeInput_max'] )
            , step: ( this._options[ this._cssName2jsName(element.dataset.targetProperty) + 'step'] || this._options['rangeInput_step'] )
          }
          ;
    ...
    _p._initControls = function() {
        var sliders = this._container.getElementsByClassName(this._options['sliderControlsClass'])
          , labels = this._container.getElementsByClassName(this._options['labelControlsClass'])
          , setup = {
              optionalFeatures: [
                  this._container.getElementsByClassName(this._options['optionalFeaturesControlsClass'])
                , ['_initFeaturesControl', 'optional']]
              , defaultFeatures: [
                  this._container.getElementsByClassName(this._options['defaultFeaturesControlsClass'])
                , ['_initFeaturesControl', 'default']]
            }
          , afterInit = []
          , initFunc, initFuncArgs, containers, key
          , sliderDataOptions = ['min', 'max', 'unit', 'step']
          , i, j, k, l, d
          ;

        // iterate all property sliders and add them to the setup object
        for(j=0;j<sliders.length;j++) {
            var cssProperty = sliders[j].dataset.targetProperty
              , cssName = this._cssName2jsName(cssProperty)
              , sliderPropertyName = [cssName, '_slider'].join('');

            this._controls[sliderPropertyName] = [sliders[j]];

            // from any slider extract any possibly set default values for the sliderDataOptions
            for(d in sliderDataOptions) {
                if(sliders[j].dataset[sliderDataOptions[d]])
                    this._options[cssName + '_' + sliderDataOptions[d]] = sliders[j].dataset[sliderDataOptions[d]];
                else
                    this._options[cssName + '_' + sliderDataOptions[d]] = this._options['rangeInput_' + sliderDataOptions[d]];
            }

            // from any slider extract any possibly set default "value" attribute
            this._options['rangeInput_' + cssName + '_value'] = sliders[j].dataset.value;
            setup[sliderPropertyName] = [[sliders[j]], ['_initSlider']];
        }

        // iterate through all slider labels
        for(k=0;k<labels.length;k++) {
            var cssProperty = labels[k].dataset.targetProperty
              , cssName = this._cssName2jsName(cssProperty)
              , labelPropertyName = [cssName, '_label'].join('');

          this._controls[labelPropertyName] = [labels[k]];

          // pass a third parameter callback function that after all other setup is called and
          // sets the passed in or default value
          setup[labelPropertyName] = [
              [labels[k]]
            , ['_initLabel']
            , this._setCssValue.bind(this, cssProperty 
              , (this._options['rangeInput_' + cssName + '_value'] || this._options['rangeInput_value']))
          ];
        }

        // iterate over all of the setup object and initiate based on passed in initFunc's
        for(key in setup) {
    ...
    _p._setCssValue = function(property, value) {
        var i, l
          ...
          , valueUnit = [value, (this._options[propertyJsName + '_unit'] || this._options['rangeInput_unit'])]
          , propertyValueBaseName = this._cssName2jsName(property)
          ;

        // loop over all sliders and update their input's values
        for(i=0,l=this._controls[propertyValueBaseName + '_slider'].length;i<l;i++) {
            this._options.setCssValueToInput.call(
                this, this._controls[propertyValueBaseName + '_slider'][i].childNodes[0], value
            );
        }

        // loop over all labels and update their text content
        for(i=0,l=this._controls[propertyValueBaseName + '_label'].length;i<l;i++) {
            this._controls[propertyValueBaseName + '_label'][i].textContent = valueUnit.join(' ');
        }
