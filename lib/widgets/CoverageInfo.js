define([
  ...
  , dom
    ...
    function _convertValue(value, highValControl) {
    ...
    _p._initFilterItems = function(key, selector) {
        ...
        for(i=0,l=items.length;i<l;i++) {
            ...
            if(i === 0) {
                rawValue = elem.value;
                this._filterValues[key] = fromRawValue(rawValue, key==='max');
            }
            else
    ...
    _p._initAllFilterItems = function() {
        var items = {
            min: this._initFilterItems('min', this._options.minFilterSelector, this._filterValues.min)
          , max: this._initFilterItems('max', this._options.maxFilterSelector, this._filterValues.max)
        };
        return items;
    };
