...
define([
    ...
    _p._onAllFontsLoaded = function(/* numberAllFonts */) {
        var fonts = this._fontsData.getFontIdsInFamilyOrder();
        for(let contentElement of this._contentElements)
            contentElement.parentNode.removeChild(contentElement);
        this._contentElements.splice(0, Infinity);

        for(let i=0,l=fonts.length;i<l;i++) {
            let items = this._createItems(i, fonts);
